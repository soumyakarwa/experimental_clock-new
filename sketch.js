...
function setup() {
  ...
  explanation_location = [speed_location[0], time_location[1]+2*margin, speed_location[0]+5*margin, speed_location[1]+sky_size]; 

  explanation = "I've always been fascinated with the relationship between light & time. And how sometimes, the stars we see in the night sky might have already died, because they might be so far away that their light has taken years to reach us. I find it really profound that light (or waves itself) creates a difference between time at which things happen and the time at which we are able to see them."
...
function draw() {
  ...
  for(i = 0; i < number_of_stars; i++){
    if(frameCount > star_distance[i]/velocity * 10){
      noStroke(); 
      fill(255, 255, 255, random(150, 255)); 
      ellipse(star_location_x[i], star_location_y[i], star_size[i]); 
      fill(0); 
  ...
  let distance = velocity * frameCount / 10; 
  ...
  text(velocity + "x 10^9 m", speed_location[0], speed_location[1]+margin/2)
  text(+ distance + " x 10^9 m", distance_location[0], distance_location[1]+margin/2); 
